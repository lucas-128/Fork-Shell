...
expand_environ_var(char *arg)
{
	...
	char *var_expanded = getenv(arg + 1);
	...
	if (strlen(var_expanded) == 0) {
		return NULL;
	}

	size_t arg_size = strlen(arg);
	size_t expanded_size = strlen(var_expanded);

	if (expanded_size > arg_size) {
		char *new_var = realloc(arg, expanded_size + 1);
		strcpy(new_var, var_expanded);
		arg = new_var;
		return arg;
	}

	strcpy(arg, var_expanded);
	return arg;
}
